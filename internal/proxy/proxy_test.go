...
func TestGeminiBridgeMapsCache(t *testing.T) {
	...
	up := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Authorization") != "" {
			t.Error("native Gemini must not send Bearer")
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{
	...
	t.Setenv("DECODED_GEMINI_BASE_URL", up.URL)
	t.Setenv("DECODED_ANTHROPIC_BASE_URL", "")
	t.Setenv("DECODED_UPSTREAM_PROFILE", "")
	...
	req, err := http.NewRequest(http.MethodPost, pxy.URL+"/v1/chat/completions", bytes.NewReader(body))
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Content-Type", "application/json")
	...
	if !strings.Contains(gotPath, "/v1beta/models/gemini-3.6-flash:generateContent") {
		t.Fatalf("path=%s", gotPath)
	}
	if gotKey != "test-key" {
		t.Fatalf("api key=%s", gotKey)
	}
	if !bytes.Contains(gotBody, []byte(`"system_instruction"`)) {
		t.Fatalf("upstream body missing system: %s", gotBody)
	}
	if !bytes.Contains(out, []byte(`"cached_tokens":4100`)) {
		t.Fatalf("client body missing cached_tokens: %s", out)
	}
	if !bytes.Contains(out, []byte(`handleAPI`)) {
		t.Fatalf("client body missing reply: %s", out)
	}
}
